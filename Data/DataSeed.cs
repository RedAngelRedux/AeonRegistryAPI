...
public class DataSeed
{
    ...
    public static async Task ManageDataAsync(IServiceProvider svcProvider)
    {
        ...
        await SeedArtifactMediaFilesAsync(svcProvider);
    ...
    public static async Task SeedCatalogRecordsAsync(IServiceProvider svcProvider)
    {
        ...
        foreach (var fileName in files)
        {
            ...
            foreach (var recordImport in records)
            {
                ...
                // Set status from import data with graceful fallback
                if (!catalogRecord.TrySetStatusFromString(recordImport.Status, out var statusError))
                {
                    Console.WriteLine($"⚠️ Warning: {statusError} Defaulting to Draft.");
                    // Status is already set to Draft above
                }
                ...
                foreach (var noteImport in recordImport.Notes)
                {
                    ...
                    catalogRecord.Notes.Add(new CatalogNote
                    {
                        ...
                        DateSubmitted = noteImport.Created
                    });
                }
    ...
    public static async Task SeedArtifactMediaFilesAsync(IServiceProvider svcProvider)
    {
        var dbContext = svcProvider.GetRequiredService<ApplicationDbContext>();
        var userManager = svcProvider.GetRequiredService<UserManager<ApplicationUser>>();

        if (await dbContext.ArtifactMediaFiles.AnyAsync())
        {
        ...
        var adminUser = await userManager.FindByEmailAsync("admin@aeon.org");
        if (adminUser == null)
        {
            Console.WriteLine("Admin user not found. Skipping artifact media seeding.");
            return;
        }
        ...
        foreach (var file in Directory.GetFiles(imagesPath))
        {
            ...
            dbContext.ArtifactMediaFiles.Add(new ArtifactMediaFile
            {
                ArtifactId = artifact.Id,
